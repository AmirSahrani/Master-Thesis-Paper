...
mind become hard. In the case of $\domain{Condorcet}$ it might be reasonable to
make use of a rule such as Black's rule
...
at hereditary strategyproof domains. We present the single-peaked domain, will
also be the main focus of this thesis. This is the domain of all single-peaked
...
clear to see that, given that there is exactly 1 \emph{issue}-dimension,
...
consist of 2 semantic issues, with only 1 issue voters simply reach substantive
...
title \citet{radDeliberationSinglePeakednessCoherent2021} model deliberation
...
exaggerate the distance when there are many candidates, for example if 2 voters
agree on the relative ranking of all but 1 candidate, which one voter happens
...
Where $b$ is this voter's bias, and $d_s$ is the distance between two profiles
