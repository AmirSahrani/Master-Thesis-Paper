...
\Cref{fig:rep_cyclic} for biases less than 0.73, all metric result in acyclic
...
cyclic. This is further illustrated in \Cref{fig:rep_count}, showing that
...
the metrics introduced in \Cref{sec: replication}.
...
Furthermore, \Cref{fig:binned_errors} shows that the model has lower MAE when
...
smaller bins. \Cref{fig:binned_errors} shows the progression of errors over
...
change in PBS for healthcare very well across different, as well as at least
...
any of the methods of generating the trust matrices . For the environment and
foreign policy, however, the model predicted an increase in PBS on average,
...
similarity seems to result in little change at all.
...
\begin{figure}[ht]
	...
	\end{center}
	\caption{
		Change in PBS per topic. This figure compares the observed change in PBS across policy topics (e.g., healthcare, immigration), showing the average change for three trust mechanisms: ego-based, similarity-based, and the model using all trust signals combined.
...
\Cref{fig:bias_slowdown} shows the relation between the bias factor and the PBS
